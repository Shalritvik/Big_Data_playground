...
# --------------------------------
    ...
    value = int(value)
    #int() will convert a string to integer (this code has no error checking)
    # ---------------------------------
    # Key Check part
    ...
    #check if key has changed ('==' is logical equality check)
        running_total += value # add value to running total
    else:
        if last_key: # if this key that was just read in is different, and the
                    # previous (i.e., last) key is not empty,
